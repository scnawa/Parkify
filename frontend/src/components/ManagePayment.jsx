...
function ManagePayment(props) {
    ...
    React.useEffect(() => {
        ...
        fetchCards().then((data)=>{
            ...
            setdefaultCard(data.default_payment);
        }).catch(console.log);
        console.log(cards);
    ...
    return(
        <ThemeProvider theme={theme}>
            <Box display='flex' flexDirection="column" sx={{ justifyContent: "space-between", margin: 2 }}>
                ...
                <List sx={{ width: '100%', bgcolor: 'background.paper',   borderRadius: 1.5,
                    border: '1px solid',
                    borderColor: 'divider',
                }}>
                    {cards.map((card, index) => (
                        <>
                            <ListItem key={card.id + defaultCard} sx={{
                                ...
                                <CustomerCard token={token} card={card} setdefaultCard={setdefaultCard} defaultCard={defaultCard} cards={cards} setCards={setCards} />
                            </ListItem>
